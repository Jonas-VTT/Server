...
exports.updateElement = async (req, res) => {
   try {
      const { id, elementId } = req.params // ID da Cena e ID do Elemento
      const updates = req.body // { x: 100, y: 200 }

      // Monta o objeto de update dinâmico
      // Ex: "elements.$.x": 100, "elements.$.y": 200
      const updateFields = {}
      ...
      const updatedScene = await Scene.findOneAndUpdate(
         { _id: id, "elements.id": elementId }, // Busca CENA pelo ID e ELEMENTO pelo ID
         { $set: updateFields },
         { new: true }
      )
...
exports.removeElement = async (req, res) => {
   try {
      ...
      const updatedScene = await Scene.findByIdAndUpdate(
         id,
         { $pull: { elements: { id: elementId } } }, // $pull remove do array
         { new: true }
      )
